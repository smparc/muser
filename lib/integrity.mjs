...
export async function roomIntegrity(db, m, n = NOW()) {
  ...
  return {
    ...
    limits: {
      min_chars: MIN_CHARS,
      note: `Authorship detection is least reliable on short, plain text: a message under ${MIN_CHARS} characters gets no verdict at all, and a brief factual one can read as human even when a model wrote it. Profiles are the stronger signal here; treat a single message label as weak evidence.`,
      provider: 'GPTZero',
    },
  };
}
