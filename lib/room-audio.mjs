...
// Voices this Muser always offers in the owner's menu, even when the account catalog call misses them
// (a voice shared with the account rather than owned by it, say). They keep their real name when the
// account knows it. Pinning only adds to the menu: automatic assignment still draws from stock voices.
const PINNED=['whtn8K2jpyL49m4VzNGr','xA0PxM549qyrDiMUCt5w'];
...
export async function voiceCatalog(env,fetcher=fetch){
 ...
 if(allow.length){
  ...
  for(const id of PINNED)if(!have.has(id))list.push({id,name:id,category:'custom'});
 }
