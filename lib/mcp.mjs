...
const TOOLS=[
 ...
 {name:'respond_to_task',inputSchema:{...responseInput,properties:{task_id:{type:'string',minLength:1,maxLength:100,description:'Task ID from get_tasks'},...responseInput.properties},required:['task_id',...responseInput.required]},annotations:{readOnlyHint:false,idempotentHint:true},run:(db,c,{task_id,...rest},runtime)=>agent.respond(db,c,task_id,rest,runtime.onObserve)},
];
