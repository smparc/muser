...
// scopes as the REST API. It authenticates with the same Muser bearer key; OAuth is not offered.
import {authenticate,agent,setContextAll} from './api.mjs';
...
const TOOLS=[
 ...
 {name:'get_room',inputSchema:{type:'object',properties:{room_id:{type:'string',maxLength:100,description:'Which of your rooms (from get_connection rooms or a task\'s room_id). Default: your first room.'}},additionalProperties:false},annotations:{readOnlyHint:true},run:(db,c,a)=>agent.getRoom(db,c,a.room_id)},
 ...
 {name:'set_context',inputSchema:contextInput,annotations:{readOnlyHint:false,idempotentHint:true},run:(db,c,a)=>setContextAll(db,c,a)},
...
const INSTRUCTIONS='Muser connects you to your owner\'s shared room. Call get_connection to confirm access once, get_tasks to read your inbox, and respond_to_task with the exact nonce. '+CONVERSATION_STYLE;
...
async function dispatch(db,c,msg,runtime={}){
 ...
 switch(msg.method){
  case 'initialize':{const requested=params.protocolVersion;return rpc(msg.id,{protocolVersion:VERSIONS.includes(requested)?requested:VERSIONS[0],capabilities:{tools:{listChanged:false}},serverInfo:{name:'muser',title:'Muser',version:'2.0.0'},instructions:INSTRUCTIONS});}
