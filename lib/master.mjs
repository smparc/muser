// The Commonroom master: two models (OpenAI and Gemini) write the questions Muses answer and decide which people match.
...
// pair: [connectionId, connectionId] to write the opening question of a Muse ↔ Muse conversation instead of a room round.
export async function writeQuestion(ctx,providers,{pair}={}){
 if(pair){
  const [a,b]=pair.map(id=>ctx.muses.find(m=>m.connection_id===id));
  if(!a||!b)throw Error('Both Muses must be active in this room.');
  ctx={...ctx,muses:[a,b],sources:new Map([...ctx.sources].filter(([,s])=>pair.includes(s.connection_id)))};
 }
 const rules=pair?pairRules(...ctx.muses):RULES,audience=pair?'the opening question for these two Muses\' conversation':'the next question for the room';
 ...
 const propose=async feedback=>{
  const out=await proposer.json({name:'master_question',schema:proposeSchema,
   system:`You are the question writer in Commonroom's master, working with a second AI model that reviews your drafts. Write ${audience}. ${rules} Explain in one sentence which connection you are trying to test.`,
 ...
 const critique=async draft=>{
  const out=await critic.json({name:'master_review',schema:critiqueSchema,
   system:`You are the reviewer in Commonroom's master, working with a second AI model that drafts questions. Approve the draft only if it meets every rule; otherwise list the problems and give a revised question that does. ${rules}`,
...
export async function runMasterStep(db,roomId,providers,{force=false}={}){
 const n=NOW();
 ...
 try{
  ...
  if(last&&!last.evaluated_at){
   ...
   await saveMatches(db,roomId,last.round_id,await judgeMatches(ctx,providers),ctx);
  ...
  const q=await writeQuestion(ctx,providers);
