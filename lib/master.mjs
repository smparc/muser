...
export async function writeQuestion(ctx,providers){
 ...
 const propose=async feedback=>{
  const out=await proposer.json({name:'master_question',schema:proposeSchema,
   system:`You are the question writer in Commonroom's master, working with a second AI model that reviews your drafts. Write the next question for the room. ${RULES} Explain in one sentence which connection you are trying to test.`,
