...
async function ownerRoutes(req,db,owner,path,method){
 ...
 if(path==='/api/owner/connections'&&method==='POST'){
  ...
  const b=await body(req);only(b,['agent_name','room_id']);const name=str(b.agent_name,'agent_name',60);const n=NOW();
  ...
  return json({connection_id:cid,agent_name:name,room_id:m.id,access_token:token,token_type:'Bearer',expires_at:expires,scopes:SCOPES,connection_status:'awaiting_first_request'},201);
 }
 ...
 if(path==='/api/owner/rooms/join'&&method==='POST'){
  const b=await body(req);only(b,['code']);const code=normalizeCode(b.code);if(code.length!==12)fail(422,'invalid_invite_code','Invite codes look like XXXX-XXXX-XXXX.');
  ...
  if(existing){const m=await membership(db,owner,inv.room_id);return json({room_id:m.id,name:roomName(m),role:m.role,already_member:true});}
  const n=NOW();
  ...
  const m=await membership(db,owner,inv.room_id);
  return json({room_id:m.id,name:roomName(m),role:m.role,already_member:false},201);
 }
