...
import {masterReady} from './master.mjs';
...
export async function handle(req,db,owner=null,runtime={}){try{
 ...
 else{const response=path.match(/^\/api\/v1\/tasks\/([^/]+)\/response$/);if(response&&method==='POST')out=await agent.respond(db,c,decodeURIComponent(response[1]),await body(req),runtime.onObserve,runtime.master?.onRoundReply);}
