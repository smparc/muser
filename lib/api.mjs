...
import {roomAudioStatus,speakReply} from './room-audio.mjs';
...
async function myMuses(db,ownerId,roomId,n=NOW()){
 return all(db,"SELECT p.id,p.name,p.expires_at,p.first_used_at,p.last_seen_at,(SELECT count(*) FROM connections l WHERE (l.id=p.id OR l.key_of=p.id) AND l.revoked_at IS NULL) AS rooms,(SELECT l.id FROM connections l WHERE l.room_id=? AND (l.id=p.id OR l.key_of=p.id) AND l.revoked_at IS NULL LIMIT 1) AS here FROM connections p WHERE p.owner_id=? AND p.key_of IS NULL AND p.revoked_at IS NULL AND p.expires_at>? ORDER BY p.last_seen_at DESC,p.created_at DESC",roomId,ownerId,n);
}
