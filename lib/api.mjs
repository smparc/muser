...
export async function authenticate(req,db){
 ...
 const links=(await all(db,'SELECT c.*,r.name AS room_name,r.archived_at AS room_archived_at FROM connections c JOIN rooms r ON r.id=c.room_id WHERE (c.id=? OR c.key_of=?) AND c.revoked_at IS NULL ORDER BY c.key_of IS NOT NULL,c.created_at,c.id',key.id,key.id)).filter(l=>!l.room_archived_at);
...
export const agent={
 async getConnection(db,c){return {status:200,body:{connection_id:c.id,agent_name:c.name,room_id:c.room_id,connection_status:'connected',expires_at:c.expires_at,scopes:SCOPES,provider_verified:false,server_time:NOW(),
  ...
  rooms:(c.links??[c]).map(l=>({room_id:l.room_id,room_name:l.room_name??null,connection_id:l.id}))}};},
 ...
 async getTasks(db,c){
  ...
  for(const l of c.links??[c]){const out=(await agent.roomTasks(db,l)).body;n=out.server_time;tasks.push(...out.tasks.map(t=>({...t,room_id:l.room_id,room_name:l.room_name??null})));}
...
async function ownerRoutes(req,db,owner,path,method,runtime={}){
 ...
 if(path==='/api/owner/conversations'&&method==='POST'){
  ...
  if(topic===null){
   ...
   if(!written)fail(503,'master_unavailable','Add GEMINI_API_KEY to the server so Gemini can write the conversation question.');
