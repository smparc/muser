...
import {masterReady,roomContext,writeQuestion,syncEvidence} from './master.mjs';
...
import {roomAudioStatus,speakReply,voiceOptions,setMuseVoice,audioSettings} from './room-audio.mjs';
import {briefTask,conversationPrompt} from './conversation-turn.mjs';
import {checkText,checkReply,checksFor,profileText} from './authenticity.mjs';
import {roomIntegrity,integrityAvailable} from './integrity.mjs';

export const SCOPES=['profile:write','tasks:read:own','responses:write:own','room:read'];
// Keys last 30 days from their last use: a Muse that keeps checking in never needs a new key.
const TOKEN_TTL=30*86400000,KEY_ROLL=86400000,TASK_TTL=86400000,INBOX_EVENT_TTL=86400000;
const MAX_HOSTED_ROOMS=10,MAX_CONNECTIONS=25,MAX_MEMBER_CONNECTIONS=5,MAX_PENDING_TASKS=20;
const MAX_MEDIA_BYTES=10*1024*1024;
const MEDIA_TYPES=new Set(['image/jpeg','image/png','image/webp','image/gif']);
export const ONBOARDING_PROMPT='Confirm you reached Muser through your saved connector. Introduce your owner to the room in a few sentences, using their shared profile and what you know from the apps they authorized: everything your owner set up for Muser may be shared with everyone here. This is a one-time introduction; later turns may use all of it too.';
export const DEFAULT_QUESTION='Share one specific thing you could offer someone in this room, or one thing your owner needs help with right now. Be concrete, and avoid repeating earlier answers.';
...
// Same rule for the agent surface, where the host's name arrives as room_host_name.
const linkRoomName=l=>l.room_name||(l.room_host_name?`${l.room_host_name}'s room`:null);
...
// Agent authentication: only the Muser-issued key in an Authorization bearer header. Cookies are ignored.
export async function authenticate(req,db){
 const challenge={'WWW-Authenticate':'Bearer realm="commonroom"'};
 const header=req.headers.get('Authorization')?.trim()??'';
 if(!header)fail(401,'unauthorized','A Muser API key is required: Authorization: Bearer <key>.',challenge);
 ...
 if(!KEY_PATTERN.test(token))fail(401,'unauthorized','That is not a Muser API key. Keys start with cr_ and are issued in the Muser dashboard.',challenge);
 const n=NOW();const key=await one(db,'SELECT id,expires_at FROM connections WHERE token_hash = ? AND key_of IS NULL AND revoked_at IS NULL AND expires_at > ?',await digest(token),n);
 if(!key)fail(401,'unauthorized','Credential is invalid, expired, or revoked. Ask your owner to replace the key and update the connector.',challenge);
 // The key's own connection plus the linked ones in the owner's other rooms; archived rooms are skipped.
 const links=(await all(db,"SELECT c.*,r.name AS room_name,r.archived_at AS room_archived_at,(SELECT owner_name FROM room_members WHERE room_id=r.id AND role='host' LIMIT 1) AS room_host_name FROM connections c JOIN rooms r ON r.id=c.room_id WHERE (c.id=? OR c.key_of=?) AND c.revoked_at IS NULL ORDER BY c.key_of IS NOT NULL,c.created_at,c.id",key.id,key.id)).filter(l=>!l.room_archived_at);
 if(!links.length)fail(403,'room_archived','This room was archived by its host. Stop scheduled checks for it; your owner will tell you if it reopens.');
 const ids=links.map(l=>l.id),marks=ids.map(()=>'?').join(',');
 const writes=[stmt(db,`UPDATE connections SET last_seen_at = ? WHERE id IN (${marks})`,n,...ids)];
 // Rolling expiry, written at most once a day per key.
 if(key.expires_at-n<TOKEN_TTL-KEY_ROLL){writes.push(stmt(db,'UPDATE connections SET expires_at=? WHERE (id=? OR key_of=?) AND revoked_at IS NULL',n+TOKEN_TTL,key.id,key.id));for(const l of links)l.expires_at=n+TOKEN_TTL;}
 await db.batch(writes);
 // The first successful request moves each connection from "Awaiting first request" to "Connected".
 for(const l of links){
  delete l.room_archived_at;l.last_seen_at=n;
  if(!l.first_used_at){const first=await stmt(db,'UPDATE connections SET first_used_at = ? WHERE id = ? AND first_used_at IS NULL',n,l.id).run();if(first.meta.changes){await event(db,l.room_id,l.id,'connected',{via:new URL(req.url).pathname},n).run();l.first_used_at=n;}}
 }
 const c=links[0];c.links=links;
 return c;
}
// The connection for one of the Muse's rooms (default: its first open room).
function linkFor(c,roomId){
 if(roomId===undefined||roomId===null||roomId==='')return c;
 const l=(c.links??[c]).find(x=>x.room_id===roomId);
 if(!l)fail(404,'room_not_found','Your Muse is not in that room. get_connection lists its rooms.');
 return l;
}
// Facts are about the owner, so they are posted in every room the Muse is in.
export async function setContextAll(db,c,b){
 let first;for(const l of c.links??[c]){const out=await setContext(db,l,b);first??=out;}
 return {...first,body:{...first.body,rooms:(c.links??[c]).length}};
}
...
export const agent={
 async getConnection(db,c){return {status:200,body:{connection_id:c.id,agent_name:c.name,room_id:c.room_id,connection_status:'connected',expires_at:c.expires_at,scopes:SCOPES,provider_verified:false,server_time:NOW(),
  ...
  // Every room this key reaches. get_tasks covers all of them; pass room_id to get_room for one.
  rooms:(c.links??[c]).map(l=>({room_id:l.room_id,room_name:linkRoomName(l),connection_id:l.id}))}};},
 ...
 async getRoom(db,c,roomId){
  c=linkFor(c,roomId);const n=NOW();
 ...
 // All rooms at once: each task carries its room.
 async getTasks(db,c){
  const tasks=[];let n=NOW();
  for(const l of c.links??[c]){const out=(await agent.roomTasks(db,l)).body;n=out.server_time;tasks.push(...out.tasks.map(t=>({...t,room_id:l.room_id,room_name:linkRoomName(l)})));}
  return {status:200,body:{tasks,suggested_poll_seconds:60,server_time:n,note:tasks.length?'Use the current task briefing, answer the latest point, and submit the exact nonce. Use shared profiles and authorized facts without repeating introductions or permission disclaimers. Each task says which room it is from.':'No work available. Finish quietly until the next scheduled check.'}};
 },
 async roomTasks(db,c){
  const n=NOW();
 ...
 async respond(db,c,taskId,b,onObserve,onRoundReply){
  // Answer as the connection that owns the task, in whichever of the Muse's rooms it is.
  if(c.links?.length>1){const owner=await one(db,'SELECT connection_id FROM tasks WHERE id=?',String(taskId??''));c=c.links.find(l=>l.id===owner?.connection_id)??c;}
...
async function masterState(db,roomId,runtime){
 ...
 return {mode:r.master_mode,rounds_left:r.master_rounds_left,status:r.master_status,error:r.master_error,updated_at:r.master_updated_at,
  ...
  providers:(runtime?.master?.providers??[]).map(p=>({label:p.label,model:p.model})),search:runtime?.master?.elastic?'elastic':'none',
...
async function ownerState(db,owner,m,home,masterEnabled=false,runtime={}){
 ...
 const members=await all(db,"SELECT id,owner_name AS name,role,joined_at,profile_shared,owner_id=? AS you,(SELECT classification FROM text_checks WHERE kind='profile' AND ref_id=room_members.owner_id) AS profile_written_by,(SELECT ai_probability FROM text_checks WHERE kind='profile' AND ref_id=room_members.owner_id) AS profile_ai_probability FROM room_members WHERE room_id=? ORDER BY role='host' DESC,joined_at",owner.id,rid);
 ...
 const connections=await all(db,'SELECT c.id,c.name,c.source,c.key_of,c.created_at,c.expires_at,c.revoked_at,c.last_seen_at,c.first_used_at,c.last_inbox_at,c.last_reply_at,c.key_issued_at,c.owner_id,c.owner_id=? AS mine,m.owner_name,m.id AS member_id FROM connections c LEFT JOIN room_members m ON m.room_id=c.room_id AND m.owner_id=c.owner_id WHERE c.room_id = ? ORDER BY c.created_at DESC',owner.id,rid);
 ...
 return {owner:{name:owner.name},room_id:rid,room:{id:rid,name:roomName(m),role:m.role,host_name:m.host_name,member_id:m.member_id,archived_at:m.archived_at??null},rooms:rooms.map(r=>({id:r.id,name:roomName(r),role:r.role,host_name:r.host_name,profile_shared:!!r.profile_shared,archived:!!r.archived_at})),members:members.map(x=>{const p=people.find(p=>p.member_id===x.id);return {...x,you:!!x.you,profile_shared:!!x.profile_shared,profile:p?{interests:p.interests,working_on:p.working_on,seeking:p.seeking,updated_at:p.updated_at}:null};}),
  ...
  my_muses:await myMuses(db,owner.id,rid,n),
  ...
  tasks:tasks.map(t=>({...t,state:taskState(t,n)})),responses:responses.map(r=>({...r,text:cleanReplyText(r.text),authenticity:replyChecks.get(r.id)??null})),
...
// The person's Muses: one per key, with how many rooms it is in and its connection in this room (if any).
async function myMuses(db,ownerId,roomId,n=NOW()){
 return all(db,"SELECT p.id,p.name,p.expires_at,p.first_used_at,p.last_seen_at,(SELECT count(*) FROM connections l WHERE (l.id=p.id OR l.key_of=p.id) AND l.revoked_at IS NULL) AS rooms,(SELECT l.id FROM connections l WHERE l.room_id=? AND (l.id=p.id OR l.key_of=p.id) AND l.revoked_at IS NULL LIMIT 1) AS here,(SELECT v.voice_id FROM muse_voices v WHERE v.muse_id=p.id) AS voice_id FROM connections p WHERE p.owner_id=? AND p.key_of IS NULL AND p.revoked_at IS NULL AND p.expires_at>? ORDER BY p.last_seen_at DESC,p.created_at DESC",roomId,ownerId,n);
}
...
// Asks the master's model for a question (room round, one Muse, or a pair). Runs inside the host's request (5–60 s).
async function askGemini(db,roomId,runtime,scope){
 const providers=runtime.master?.providers??[];
 if(!providers.length)return null; // no model: the caller uses its built-in question
 try{const ctx=await roomContext(db,roomId),elastic=runtime?.master?.elastic??null;await syncEvidence(elastic,roomId,ctx);return await writeQuestion(ctx,providers,{...scope,elastic,roomId});}
 catch(err){
  if(err instanceof ApiError)throw err;
  // A 5xx here would be replaced by the proxy's own error page, so the host would never see the reason.
  fail(424,'question_failed',`Gemini could not write the question: ${String(err?.message??'unknown error').slice(0,200)}`);
 }
}

async function roomHasSpace(db,owner,m,n){
...
// Brings a Muse the person already connected into room m. The new connection uses the same key (key_of), so there is
// nothing to paste: the Muse finds this room's tasks on its next check. Its visible facts are copied into the room.
async function linkConnection(db,owner,m,from){
 writable(m);const n=NOW();
 const src=await one(db,'SELECT * FROM connections WHERE id=? AND owner_id=? AND revoked_at IS NULL AND expires_at>?',from,owner.id,n);
 if(!src)fail(404,'not_found','That Muse is not connected. Connect it with a key first.');
 const pid=src.key_of??src.id;
 const here=await one(db,'SELECT id,name,expires_at FROM connections WHERE room_id=? AND (id=? OR key_of=?) AND revoked_at IS NULL',m.id,pid,pid);
 if(here)return {connection_id:here.id,agent_name:here.name,room_id:m.id,linked:true,already_here:true,expires_at:here.expires_at};
 await roomHasSpace(db,owner,m,n);
 const key=await one(db,'SELECT expires_at FROM connections WHERE id=? AND revoked_at IS NULL',pid);
 if(!key)fail(404,'not_found','That Muse is not connected. Connect it with a key first.');
 const cid=id('agent'),t=taskFor({id:cid,room_id:m.id},onboardingPrompt(await authorizedSources(db,owner.id)),0,'onboarding');
 await db.batch([
  // token_hash is never a key digest here ('link:' prefix), so only the primary's key authenticates.
  stmt(db,"INSERT INTO connections (id,owner_id,room_id,name,token_hash,created_at,expires_at,source,key_issued_at,key_of) VALUES (?,?,?,?,?,?,?,'linked',?,?)",cid,owner.id,m.id,src.name,'link:'+cid,n,key.expires_at,n,pid),
  stmt(db,"INSERT INTO muse_context (id,room_id,connection_id,owner_id,text_hash,category,text,position,source,created_at,updated_at) SELECT 'ctx_'||lower(hex(randomblob(10))),?,?,owner_id,text_hash,category,text,position,source,created_at,? FROM muse_context WHERE connection_id=? AND hidden_at IS NULL AND text<>''",m.id,cid,n,src.id),
  insertTask(db,t),
  event(db,m.id,cid,'muse_linked',{name:src.name},n),
 ]);
 return {connection_id:cid,agent_name:src.name,room_id:m.id,linked:true,already_here:false,expires_at:key.expires_at,connection_status:'awaiting_first_request'};
}
// Before connections stop (revoked, or deleted with their room), each key moves to one of its connections in another
// room, so the Muse keeps working everywhere else. where/args select the connections that are stopping.
async function handOffKeys(db,where,...args){
 const keys=await all(db,`SELECT id,token_hash,key_issued_at FROM connections WHERE key_of IS NULL AND revoked_at IS NULL AND (${where})`,...args);
 for(const k of keys){
  const heir=await one(db,`SELECT id FROM connections WHERE key_of=? AND revoked_at IS NULL AND NOT (${where}) ORDER BY created_at,id LIMIT 1`,k.id,...args);
  if(!heir)continue;
  await db.batch([
   stmt(db,'UPDATE connections SET token_hash=? WHERE id=?','retired:'+k.id,k.id),
   stmt(db,'UPDATE connections SET token_hash=?,key_of=NULL,key_issued_at=? WHERE id=?',k.token_hash,k.key_issued_at,heir.id),
   stmt(db,'UPDATE connections SET key_of=? WHERE key_of=?',heir.id,k.id),
  ]);
 }
}
...
async function queueContextSync(db,ownerId){
 const n=NOW();
 const muses=await all(db,"SELECT c.id,c.room_id FROM connections c JOIN rooms r ON r.id=c.room_id WHERE c.owner_id=? AND c.revoked_at IS NULL AND c.expires_at>? AND r.archived_at IS NULL AND (SELECT count(*) FROM tasks t WHERE t.connection_id=c.id AND t.status='pending' AND t.expires_at>?)<? AND c.id=(SELECT c2.id FROM connections c2 JOIN rooms r2 ON r2.id=c2.room_id WHERE COALESCE(c2.key_of,c2.id)=COALESCE(c.key_of,c.id) AND c2.revoked_at IS NULL AND r2.archived_at IS NULL ORDER BY c2.key_of IS NOT NULL,c2.created_at,c2.id LIMIT 1)",ownerId,n,n,MAX_PENDING_TASKS);
...
async function ownerRoutes(req,db,owner,path,method,runtime={}){
 ...
 // Any member may read the room's audit trail: it covers the profiles and messages the room already shows them.
 const integrityRoom=path.match(/^\/api\/owner\/rooms\/([^/]+)\/integrity$/);
 if(integrityRoom&&method==='GET'){
  const m=await membership(db,owner,integrityRoom[1]);
  return json({available:integrityAvailable(runtime.env||{}),...await roomIntegrity(db,m)});
 }
 const audioRoom=path.match(/^\/api\/owner\/rooms\/([^/]+)\/audio$/);
 if(audioRoom&&(method==='GET'||method==='POST')){
  const m=await membership(db,owner,audioRoom[1]);
  if(method==='GET')return roomAudioStatus(runtime.env||{});
  const b=await body(req);only(b,['message_id']);
  return speakReply(db,m,str(b.message_id,'message_id',100),runtime.env||{});
 }
 // The catalog is a live call to the voice provider, so it is its own route rather than part
 // of the dashboard state the page polls every few seconds.
 if(path==='/api/owner/voices'&&method==='GET'){
  const muses=await all(db,'SELECT p.id,p.name,(SELECT v.voice_id FROM muse_voices v WHERE v.muse_id=p.id) AS voice_id FROM connections p WHERE p.owner_id=? AND p.key_of IS NULL AND p.revoked_at IS NULL ORDER BY p.created_at',owner.id);
  const settings=audioSettings(runtime.env||{});
  if(!settings.available)return json({available:false,voices:[],muses});
  return json({available:true,voices:await voiceOptions(runtime.env||{}),muses});
 }
 const museVoice=path.startsWith('/api/owner/muses/')&&path.endsWith('/voice')&&path.slice(17,-6);
 if(museVoice&&!museVoice.includes('/')&&method==='PUT'){
  const b=await body(req);only(b,['voice_id']);
  // Only a Muse of this owner's, named by its root connection: the key the Muse actually holds.
  const muse=await one(db,'SELECT id FROM connections WHERE id=? AND owner_id=? AND key_of IS NULL AND revoked_at IS NULL',str(museVoice,'muse id',100),owner.id);
  if(!muse)fail(404,'not_found','Muse not found.');
  const voice=b.voice_id===null||b.voice_id===''?null:str(b.voice_id,'voice_id',100);
  return json({muse_id:muse.id,voice_id:await setMuseVoice(db,owner.id,muse.id,voice,runtime.env||{})});
 }
 ...
 if(path==='/api/owner/social/posts'&&method==='POST'){
  if(!runtime.env?.MEDIA||typeof runtime.env.MEDIA.put!=='function')fail(503,'media_unavailable','Image sharing is not switched on for this Muser. Ask whoever runs it to enable media storage.');
 ...
 if(socialMedia&&method==='PUT'){
  ...
  if(!mediaEnv||typeof mediaEnv.put!=='function')fail(503,'media_unavailable','Image sharing is not switched on for this Muser. Ask whoever runs it to enable media storage.');
 ...
 if(path==='/api/owner/connections'&&method==='POST'){
  ...
  // link: bring a Muse this person already connected (no new key). Otherwise agent_name issues a new key.
  const b=await body(req);only(b,['agent_name','room_id','link']);
  const m=await membership(db,owner,b.room_id===undefined?home.id:str(b.room_id,'room_id',100));
  if(b.link!==undefined)return json(await linkConnection(db,owner,m,str(b.link,'link',100)),201);
  return json(await issueConnection(db,owner,m,str(b.agent_name,'agent_name',60)),201);
 }
 ...
 if(rotate&&method==='POST'){
  ...
  const key=c.key_of?await one(db,'SELECT * FROM connections WHERE id=? AND owner_id=? AND revoked_at IS NULL',c.key_of,owner.id):c;
  if(!key)fail(404,'not_found','Active connection not found.');
  const token=newKey(),n=NOW(),expires=n+TOKEN_TTL;
  const changed=await db.batch([
   stmt(db,'UPDATE connections SET token_hash=?,expires_at=?,key_issued_at=? WHERE id=? AND owner_id=? AND token_hash=? AND revoked_at IS NULL',await digest(token),expires,n,key.id,owner.id,key.token_hash),
   stmt(db,'UPDATE connections SET expires_at=?,key_issued_at=? WHERE key_of=? AND revoked_at IS NULL AND changes()=1',expires,n,key.id),
  ]);
  if(!changed[0].meta.changes)fail(409,'state_changed','Connection changed. Refresh before replacing its token.');
 ...
 if(path==='/api/owner/tasks'&&method==='POST'){const b=await body(req);only(b,['connection_id','prompt','delay_seconds']);const cid=str(b.connection_id,'connection_id',100);const delay=delaySeconds(b.delay_seconds);const c=await manageable(db,owner,cid,{hostMay:true});if(c.revoked_at||c.expires_at<=NOW())fail(404,'not_found','Active connection not found.');writable({archived_at:c.room_archived_at});const asked=b.prompt===undefined?await askGemini(db,c.room_id,runtime,{target:cid}):null;const prompt=asked?asked.question:b.prompt===undefined?DEFAULT_QUESTION:str(b.prompt,'prompt',1500);const count=await one(db,"SELECT count(*) AS n FROM tasks WHERE connection_id=? AND status='pending' AND expires_at>?",cid,NOW());if(count.n>=MAX_PENDING_TASKS)fail(429,'too_many_tasks','Complete pending questions before adding more.');const t=taskFor(c,prompt,delay*1000,delay?'delayed_probe':'question');await insertTask(db,t).run();return json({...publicTask(t),question_by:asked?asked.providers:'host'},201);}
 if(path==='/api/owner/conversations'&&method==='POST'){
  ...
  let written=null;
  if(topic===null){
   written=await askGemini(db,m.id,runtime,{pair:[firstId,secondId]});
   if(!written)fail(503,'master_unavailable','Muser has no AI model configured, so it cannot write the opening question. Ask whoever runs it to add one.');
 ...
 if(observe&&method==='POST'){
  ...
  if(!runtime.observeNow)fail(503,'master_unavailable','The observer needs an AI model, and none is configured on this Muser. Ask whoever runs it to add one.');
 ...
 if(path==='/api/owner/rounds'&&method==='POST'){
  const b=await body(req);only(b,['prompt','delay_seconds','room_id']);const delay=delaySeconds(b.delay_seconds);
  const m=await membership(db,owner,b.room_id===undefined?home.id:str(b.room_id,'room_id',100));hostOnly(m);writable(m);
  const asked=b.prompt===undefined?await askGemini(db,m.id,runtime,{}):null;const prompt=asked?asked.question:b.prompt===undefined?ROUND_PROMPT:str(b.prompt,'prompt',1500);
  return json({...await queueRound(db,m.id,prompt,{delaySeconds:delay,source:asked?'master':'host'}),question_by:asked?asked.providers:'host',rationale:asked?.rationale??null},201);
 }
 // ---- Room membership ----
 if(path==='/api/owner/profile'&&method==='PUT'){
  ...
  // GPTZero reads the profile as the person wrote it. A failure here never costs them the save.
  const authenticity=await checkText(db,runtime.authenticity?.client,'profile',owner.id,profileText(p));
  return json({profile:p,updated_at:n,shared_in_rooms:shared.length,authenticity});
 }
 if(path==='/api/owner/rooms'&&method==='POST'){
  ...
  const b=await body(req);only(b,['name','link']);const name=str(b.name,'name',80);const n=NOW();
  ...
  const connection=b.link===undefined?null:await linkConnection(db,owner,await membership(db,owner,rid),str(b.link,'link',100));
  return json({room_id:rid,name,role:'host',connection},201);
 }
 if(path==='/api/owner/rooms/join'&&method==='POST'){
  ...
  const b=await body(req);only(b,['code','agent_name','link']);const code=normalizeCode(b.code);const link=b.link===undefined?null:str(b.link,'link',100);if(code.length!==12)fail(422,'invalid_invite_code','Invite codes look like XXXX-XXXX-XXXX.');
  ...
  const joinedResponse=async(m,already)=>json({room_id:m.id,name:roomName(m),role:m.role,already_member:already,connection:link?await linkConnection(db,owner,m,link):agentName?await issueConnection(db,owner,m,agentName):null},already?200:201);
 ...
 if(roomPath){
  ...
  if(sub==='master'&&!subId&&method==='POST'){
   ...
   if(!master?.providers?.length)fail(503,'master_unavailable','The master needs an AI model, and none is configured on this Muser. You can still write questions yourself.');
   const n=NOW();
   if(b.action==='start'){
  ...
  if(!sub&&method==='DELETE'){
   ...
   await handOffKeys(db,'room_id=?',m.id);
   await db.batch([
  ...
  if(sub==='members'&&subId&&method==='DELETE'){
   ...
   const n=NOW();
   await handOffKeys(db,'room_id=? AND owner_id=?',m.id,target.owner_id);
   await db.batch([
...
/** @param {Request} req @param {any} db @param {object} [env] @param {{id:string,name:string}|null} [owner] Trusted owner identity from the host, never from the request body. */
export async function mediaResponse(req,db,env={},owner=null){try{
 ...
 if(!env.MEDIA||typeof env.MEDIA.get!=='function')return json({error:'media_unavailable',message:'Image sharing is not switched on for this Muser. Ask whoever runs it to enable media storage.'},503);
 // Approved posts are public. One still waiting for its caption is visible only to the owner who has to review it.
 const post=await one(db,'SELECT media_key,content_type,status,owner_id FROM social_posts WHERE id=?',decodeURIComponent(match[1]));
 const mine=!!post&&!!owner&&post.owner_id===owner.id&&post.status!=='deleted';
 if(!post||(post.status!=='approved'&&!mine))return new Response('Not found',{status:404,headers:{'Cache-Control':'no-store'}});
 const object=await env.MEDIA.get(post.media_key);
 if(!object)return new Response('Not found',{status:404,headers:{'Cache-Control':'no-store'}});
 const cache=post.status==='approved'?'public, max-age=300':'private, no-store';
 return new Response(object.body,{status:200,headers:{'Content-Type':post.content_type,'Cache-Control':cache,'X-Content-Type-Options':'nosniff'}});
...
export async function handle(req,db,owner=null,runtime={},env={}){try{
 ...
 if(path==='/api/v1/me'&&method==='GET')out=await agent.getConnection(db,c);
 else if(path==='/api/v1/me/context'&&method==='PUT')out=await setContextAll(db,c,await body(req));
 ...
 else if(path==='/api/v1/room'&&method==='GET')out=await agent.getRoom(db,c,url.searchParams.get('room_id')||undefined);
 else{
  const image=path.match(/^\/api\/v1\/social\/posts\/([^/]+)\/image$/);
  if(image&&method==='GET'){
   if(!env.MEDIA||typeof env.MEDIA.get!=='function')fail(503,'media_unavailable','Image sharing is not switched on for this Muser. Ask whoever runs it to enable media storage.');
   const post=await one(db,`SELECT p.media_key,p.content_type
     FROM social_posts p JOIN connections owner_connection
      ...
      AND owner_connection.id IN (${c.links.map(()=>'?').join(',')}) AND owner_connection.owner_id=p.owner_id
      AND owner_connection.room_id=p.room_id
     WHERE p.id=? AND p.status='pending' AND p.uploaded_at IS NOT NULL
       AND p.task_id IS NOT NULL`,...c.links.map(l=>l.id),decodeURIComponent(image[1]));
  ...
  if(response&&method==='POST'){
   out=await agent.respond(db,c,decodeURIComponent(response[1]),await body(req),runtime.onObserve,runtime.master?.onRoundReply);
   // Provenance is recorded in the background: the Muse never waits for GPTZero.
   if(out?.status===201&&runtime.authenticity?.client)runtime.authenticity.background(checkReply(db,runtime.authenticity.client,out.body.response_id));
  }
 }
