...
(() => {
  ...
  const link = (view, label, symbol) => `<a href="${view === 'profile' ? '/profile.html' : view === 'provenance' ? '/integrity.html' : roomLink(view)}" data-nav="${view}" ${view === page ? 'class="active" aria-current="page"' : ''}>${icon(symbol)}<span>${label}</span></a>`;
  const rail = document.createElement('aside');
  rail.className = 'app-rail';
  rail.innerHTML = `<a class="brand-mark" href="/" aria-label="Muser home">${icon('mark')}</a><nav aria-label="Main navigation">${link('home', 'Home', 'home')}${link('muse', 'Muse', 'follow')}${link('people', 'People', 'people')}${link('spaces', 'Spaces', 'spaces')}${link('messages', 'Messages', 'message')}${link('provenance', 'Provenance', 'spark')}${link('profile', 'Profile', 'profile')}</nav><div class="rail-bottom">${link('settings', 'Settings', 'settings')}</div>`;
  ...
  if (header) {
    ...
    header.innerHTML = `<a class="brand" href="/">Muser</a><nav class="top-tabs" aria-label="Explore">${link('home', 'The commons', 'home')}${link('people', 'Discover', 'people')}${link('projects', 'Projects', 'folder')}</nav><div class="header-account"><span class="app-tag">A place to belong</span></div>`;
